...
function runSegmentationTestSuiteWithProvider(providerType: 'OpenAI' | 'Custom') {
  describe(`Profile Chat Feature (${providerType})`, () => {
    ...
    it(`should create segmentation rules through chat interaction (${providerType})`, () => {
      // Click on the chat button
      cy.get('[data-cy="profile-chat-button"]').click()
      ...
      // Type and send a message requesting segmentation rules creation
      const requestMessage = 'Please create 3 segmentation rules for identifying paragraphs in text'
      cy.get('[data-cy="message-input"]')
        .should('be.visible')
        .type(requestMessage)
      cy.get('[data-cy="send-button"]').click()
    ...
    it(`should adopt all segmentation rules when clicking adopt all button (${providerType})`, () => {
      // Click on the chat button
      cy.get('[data-cy="profile-chat-button"]').click()
      ...
      // Wait for and verify AI response with segmentation rules
      cy.get('[data-cy="assistant-message"]')
        .should('be.visible')
        .within(() => {
      ...
      // Verify the segmentation rules are visible in the profile view
      cy.get('[data-cy="datapoints-container"]').should('be.visible')
      cy.get('[data-cy="datapoint-card"]')
        .should('have.length', 3)
        .each(($point) => {
          // Verify each rule has basic content
